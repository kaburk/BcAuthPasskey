...
| `templates/plugin/BcAdminThird/Admin/Users/login.php` | Admin ログイン画面 override（パスキーボタン＋BcSocialAuth ボタン統合） |
| `templates/plugin/BcFront/Users/login.php` | Front ログイン画面 override（パスキーボタン） |
| `config/routes.php` | Admin / Front ルート定義 |
| `webroot/js/passkey-auth.js` | Base64URL 変換・login / register フロー（WebAuthn フロントエンド実装済み） |

> **注意**: Admin ログイン画面の template override は、BcSocialAuth が同時にインストールされている場合にソーシャルログインボタンも自動表示する統合テンプレートになっています。BcSocialAuth を単体でインストールする場合は BcSocialAuth 側の独自 override が別途必要です（残タスク参照）。
