...
BcAuthPasskey は WebAuthn ライブラリをプラグイン内で管理します。
初回導入時は、プラグインディレクトリで composer install を実行してください。
...
`vendor/` と `composer.lock` はローカル開発用として git 管理対象外です。
依存ライブラリが未導入の場合、プラグインは WebAuthn 機能を登録しません。
