...
初期段階では BcPasskeyAuth を単独プラグインとして進めますが、内部設計は次のような拡張を意識します。
...
```text
plugins/
├── BcPasskeyAuth/         # WebAuthn / Passkey
├── BcSocialAuth/          # OAuth / OpenID Connect 系
└── BcAuthCommon/          # 共通の認証連携部品
```

認証完了後に baserCMS のログイン状態へ接続する処理は、`BcAuthCommon` の `AuthLoginService` と `AuthRedirectService` に切り出しています。
一方で、WebAuthn の challenge / attestation / assertion 検証は BcPasskeyAuth 側に残します。
...
このテーブルは BcPasskeyAuth に含めず、別プラグインまたは共通基盤で扱う方が責務分離しやすい想定です。
...
- `BcPasskeyAuth.loginChallenge.Admin`
- `BcPasskeyAuth.registerChallenge.Admin`
...
- GET /bc-passkey-auth/passkeys/register_challenge
- POST /bc-passkey-auth/passkeys/register
...
BcPasskeyAuth では、少なくとも 認証成功後のログイン確立 と リダイレクト判定 を後から切り出せるように実装する方針とします。
...
BcSocialAuth と同様に、初期段階では template override を採用します。
...
- BcPasskeyAuth は WebAuthn / Passkey 専用プラグインとして進める
- SNS ログインは BcSocialAuth のような別プラグインとして分離する
...
この方針は BcSocialAuth 側の設計とも整合しており、Google / X は BcSocialAuth 本体に同梱し、LINE / Apple / GitHub などは外部アドオンとして追加する前提です。

## まとめ

BcPasskeyAuth は、baserCMS の既存認証基盤を活かしながら、パスキー認証をプラグインとして追加するための設計を採用します。
